...
namespace WuGaren
{
    static class Program
    {
        ...
        static readonly float[] RMissingHealth = new float[] { 0.286f, 0.333f, 0.40f };
            ...
            if (Target != null)
            {
                if (Target.IsValidTarget())
                {
                    ...
                    if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo))
                    {
						...
						if (R.IsReady())
						{
							if (Menu["UseRCombo"].Cast<CheckBox>().CurrentValue && Target.IsValidTarget(R.Range) && SpellDamage(SpellSlot.R) > Target.Health + 30)
							{
								if (Player.HasBuff("GarenE")) E.Cast();
								R.Cast(Target);
							}
        ...
        static float SpellDamage(SpellSlot slot)
        {
